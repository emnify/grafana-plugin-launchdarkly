    ...
    const promises = options.targets.map((query) =>
      ...
      this.doRequest(defaults(query, defaultQuery), options).then((response) => {
        ...
        if (response.data._links.next) {
          throw new Error(
            'More than 20 items returned from LaunchDarkly API for selected time range. Please choose smaller range, as pagination support for LD API is not implemented, yet.'
          );
        }
        ...
        response.data.items.forEach((item: any) => {
          ...
          frame.appendRow([item.date, text, item.kind, tags, item._id]);
        });
